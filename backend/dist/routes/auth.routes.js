...
const generateToken = (userId) => {
    return jsonwebtoken_1.default.sign({ id: userId }, process.env.JWT_SECRET || 'defaultsecret', { expiresIn: '30d' });
};
...
router.post('/register', (req, res, next) => {
    const validationChains = (0, express_validator_1.checkSchema)(registerSchema);
...
router.post('/login', (req, res, next) => {
    const validationChains = (0, express_validator_1.checkSchema)(loginSchema);
    Promise.all(validationChains.map(chain => chain.run(req)))
        .then(() => next())
        .catch(next);
}, handleValidationErrors, async (req, res) => {
...
// Update user profile
router.patch('/me', auth_1.protect, (req, res, next) => {
    ...
    try {
        ...
        const user = await User_1.User.findByIdAndUpdate(req.user?._id, { $set: updates }, { new: true, runValidators: true }).select('-password');
        res.json({
            success: true,
            data: user
        });
...
// Change password
router.post('/change-password', auth_1.protect, (req, res, next) => {
    const validationChains = (0, express_validator_1.checkSchema)({
        currentPassword: {
            notEmpty: true,
